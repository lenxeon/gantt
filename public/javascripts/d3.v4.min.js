! function() {
  ...
  d3_class(d3_Map, {
    ...
    forEach: function(f) {
      for (var key in this)
        if (key.charCodeAt(0) === d3_map_prefixCode) f.call(this, key.substring(
          1), this[key]);
    }
  });
  ...
  d3_class(d3_Set, {
    ...
    forEach: function(f) {
      for (var value in this)
        if (value.charCodeAt(0) === d3_map_prefixCode) f.call(this, value
          .substring(1));
    }
  });
  d3.behavior = {};
  ...
  function d3_event_dragSuppress() {
    var name = ".dragsuppress-" + ++d3_event_dragId,
      click = "click" + name,
      w = d3.select(d3_window).on("touchmove" + name, d3_eventPreventDefault).on(
  ...
  d3.behavior.drag = function() {
    var event = d3_eventDispatch(drag, "drag", "dragstart", "dragend"),
      origin = null,
      mousedown = dragstart(d3_noop, d3.mouse, d3_behavior_dragMouseSubject,
        "mousemove", "mouseup"),
    ...
    function dragstart(id, position, subject, move, end) {
      return function() {
        var that = this,
          ...
          dragOffset, dragSubject = d3.select(subject()).on(move + dragName,
  ...
  d3.behavior.zoom = function() {
    ...
    function dblclicked() {
      ...
      zoomstarted(dispatch);
      scaleTo(Math.pow(2, d3.event.shiftKey ? Math.ceil(k) - 1 : Math.floor(k) +
        1));
      translateTo(p, l);
      zoomed(dispatch);
      zoomended(dispatch);
  ...
  function d3_lab(l, a, b) {
    return this instanceof d3_lab ? void(this.l = +l, this.a = +a, this.b = +b) :
      ...
      l instanceof d3_hcl ? d3_hcl_lab(l.l, l.c, l.h) : d3_rgb_lab((l = d3_rgb(
  ...
  function d3_xhr(url, mimeType, response, callback) {
    ...
    function respond() {
      var status = request.status,
        result;
      if (!status && request.responseText || status >= 200 && status < 300 ||
        status === 304) {
        try {
  ...
  var d3_timer_queueHead, d3_timer_queueTail, d3_timer_interval,
    d3_timer_timeout, d3_timer_active, d3_timer_frame = d3_window[
      d3_vendorSymbol(d3_window, "requestAnimationFrame")] || function(callback) {
  ...
  var d3_formatPrefixes = ["y", "z", "a", "f", "p", "n", "碌", "m", "", "k", "M",
    "G", "T", "P", "E", "Z", "Y"
  ].map(d3_formatPrefix);
  ...
  function d3_geo_areaRingStart() {
    ...
    d3_geo_area.point = function(位, 蠁) {
      ...
      位0 = (位00 = 位) * d3_radians, cos蠁0 = Math.cos(蠁 = (蠁00 = 蠁) *
          d3_radians / 2 + 蟺 / 4),
        sin蠁0 = Math.sin(蠁);
    };
    ...
    function linePoint(位, 蠁) {
      ...
      if (p0) {
        ...
        if (antimeridian ^ (s * 位_ < 位i && 位i < s * 位)) {
          ...
          if (蠁i > 蠁1) 蠁1 = 蠁i;
        } else if (位i = (位i + 360) % 360 - 180, antimeridian ^ (s * 位_ < 位i &&
            位i < s * 位)) {
          var 蠁i = -inflection[1] * d3_degrees;
          if (蠁i < 蠁0) 蠁0 = 蠁i;
        } else {
    ...
    return function(feature) {
      ...
      if (n) {
        ...
        for (var n = merged.length - 1, i = 0, a = merged[n], b; i <= n; a =
          b, ++i) {
          b = merged[i];
          if ((d位 = angle(a[1], b[0])) > best) best = d位, 位0 = b[0], 位1 = a[
            1];
        }
      }
      ranges = range = null;
  ...
  d3.geo.centroid = function(object) {
    ...
    if (m < 蔚2) {
      ...
      if (d3_geo_centroidW1 < 蔚) x = d3_geo_centroidX0, y = d3_geo_centroidY0,
        z = d3_geo_centroidZ0;
      m = x * x + y * y + z * z;
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.x)[0] < 0 ? a[1] - half蟺 - 蔚 : half蟺 - a[1]) - ((b = b.x)[0] <
      0 ? b[1] - half蟺 - 蔚 : half蟺 - b[1]);
  }
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        var 位 = point[0],
          ...
          antimeridian = ad位 > 蟺,
          k = sin蠁0 * sin蠁;
        d3_geo_areaRingSum.add(Math.atan2(k * sd位 * Math.sin(ad位), cos蠁0 * cos蠁 +
          k * Math.cos(ad位)));
    ...
    return (polarAngle < -蔚 || polarAngle < 蔚 && d3_geo_areaRingSum < 0) ^
      winding & 1;
  }
  var d3_geo_clipAntimeridian = d3_geo_clip(d3_true,
    d3_geo_clipAntimeridianLine, d3_geo_clipAntimeridianInterpolate, [-蟺, -蟺 /
      2
    ]);
  ...
  function d3_geo_clipAntimeridianIntersect(位0, 蠁0, 位1, 蠁1) {
    ...
    return abs(sin位0_位1) > 蔚 ? Math.atan((Math.sin(蠁0) * (cos蠁1 = Math.cos(蠁1)) *
        Math.sin(位1) - Math.sin(蠁1) * (cos蠁0 = Math.cos(蠁0)) * Math.sin(位0)) /
      (cos蠁0 * cos蠁1 * sin位0_位1)) : (蠁0 + 蠁1) / 2;
  }
  ...
  function d3_geo_clipCircle(radius) {
      ...
      return {
        ...
        point: function(位, 蠁) {
          var point1 = [位, 蠁],
            ...
            c = smallRadius ? v ? 0 : code(位, 蠁) : v ? code(位 + (位 < 0 ? 蟺 :
              -蟺), 蠁) : 0;
    ...
    function intersect(a, b, two) {
      ...
      if (meridian ? polar ? 蠁0 + 蠁1 > 0 ^ q[1] < (abs(q[0] - 位0) < 蔚 ? 蠁0 : 蠁1) :
        蠁0 <= q[1] && q[1] <= 蠁1 : 未位 > 蟺 ^ (位0 <= q[0] && q[0] <= 位1)) {
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return abs(p[0] - x0) < 蔚 ? direction > 0 ? 0 : 3 : abs(p[0] - x1) < 蔚 ?
        direction > 0 ? 2 : 1 : abs(p[1] - y0) < 蔚 ? direction > 0 ? 1 : 0 :
        direction > 0 ? 3 : 2;
    }
  ...
  function d3_geo_conicEqualArea(蠁0, 蠁1) {
    ...
    forward.invert = function(x, y) {
      var 蟻0_y = 蟻0 - y;
      return [Math.atan2(x, 蟻0_y) / n, d3_asin((C - (x * x + 蟻0_y * 蟻0_y) * n *
        n) / (2 * n))];
    };
    return forward;
  }
  ...
  function d3_geo_resample(project) {
    ...
    function resampleRecursive(stream) {
      ...
      function linePoint(位, 蠁) {
        var c = d3_geo_cartesian([位, 蠁]),
          p = project(位, 蠁);
        resampleLineTo(x0, y0, 位0, a0, b0, c0, x0 = p[0], y0 = p[1], 位0 = 位, a0 =
      ...
      function ringPoint(位, 蠁) {
        linePoint(位00 = 位, 蠁00 = 蠁), x00 = x0, y00 = y0, a00 = a0, b00 = b0,
          c00 = c0;
        resample.point = linePoint;
      }

      function ringEnd() {
        resampleLineTo(x0, y0, 位0, a0, b0, c0, x00, y00, 位00, a00, b00, c00,
          maxDepth, stream);
    ...
    function resampleLineTo(x0, y0, 位0, a0, b0, c0, x1, y1, 位1, a1, b1, c1,
      depth, stream) {
      var dx = x1 - x0,
        dy = y1 - y0,
        d2 = dx * dx + dy * dy;
      if (d2 > 4 * 未2 && depth--) {
        var a = a0 + a1,
          ...
          蠁2 = Math.asin(c /= m),
          位2 = abs(abs(c) - 1) < 蔚 || abs(位0 - 位1) < 蔚 ? (位0 + 位1) / 2 : Math.atan2(
            b, a),
          p = project(位2, 蠁2),
          x2 = p[0],
          y2 = p[1],
          ...
          dy2 = y2 - y0,
          dz = dy * dx2 - dx * dy2;
        if (dz * dz / d2 > 未2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 ||
          a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
          resampleLineTo(x0, y0, 位0, a0, b0, c0, x2, y2, 位2, a /= m, b /= m, c,
            depth, stream);
          stream.point(x2, y2);
          resampleLineTo(x2, y2, 位2, a, b, c, x1, y1, 位1, a1, b1, c1, depth,
            stream);
        }
      }
    }
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    projection.rotate = function(_) {
      if (!arguments.length) return [未位 * d3_degrees, 未蠁 * d3_degrees, 未纬 *
        d3_degrees
      ];
    ...
    function reset() {
      projectRotate = d3_geo_compose(rotate = d3_geo_rotation(未位, 未蠁, 未纬),
        project);
  ...
  function d3_geo_rotation(未位, 未蠁, 未纬) {
    return 未位 ? 未蠁 || 未纬 ? d3_geo_compose(d3_geo_rotation位(未位),
        d3_geo_rotation蠁纬(未蠁, 未纬)) : d3_geo_rotation位(未位) : 未蠁 || 未纬 ?
      d3_geo_rotation蠁纬(未蠁, 未纬) : d3_geo_identityRotation;
  }
  ...
  function d3_geo_rotation蠁纬(未蠁, 未纬) {
    ...
    function rotation(位, 蠁) {
      var cos蠁 = Math.cos(蠁),
        ...
        z = Math.sin(蠁),
        k = z * cos未蠁 + x * sin未蠁;
      return [Math.atan2(y * cos未纬 - k * sin未纬, x * cos未蠁 - z * sin未蠁), d3_asin(
        k * cos未纬 + y * sin未纬)];
    }
    rotation.invert = function(位, 蠁) {
      var cos蠁 = Math.cos(蠁),
        ...
        z = Math.sin(蠁),
        k = z * cos未纬 - y * sin未纬;
      return [Math.atan2(y * cos未纬 + z * sin未纬, x * cos未蠁 + k * sin未蠁),
  ...
  d3.geo.distance = function(a, b) {
    var 螖位 = (b[0] - a[0]) * d3_radians,
      ...
      cos蠁1 = Math.cos(蠁1),
      t;
    return Math.atan2(Math.sqrt((t = cos蠁1 * sin螖位) * t + (t = cos蠁0 * sin蠁1 -
      sin蠁0 * cos蠁1 * cos螖位) * t), sin蠁0 * sin蠁1 + cos蠁0 * cos蠁1 * cos螖位);
  };
  ...
  function d3_geo_lengthLineStart() {
    ...
    d3_geo_length.point = function(位, 蠁) {
      位0 = 位 * d3_radians, sin蠁0 = Math.sin(蠁 *= d3_radians), cos蠁0 = Math.cos(
        蠁);
    ...
    function nextPoint(位, 蠁) {
      var sin蠁 = Math.sin(蠁 *= d3_radians),
        ...
        cos螖位 = Math.cos(t);
      d3_geo_lengthSum += Math.atan2(Math.sqrt((t = cos蠁 * Math.sin(t)) * t + (
          t = cos蠁0 * sin蠁 - sin蠁0 * cos蠁 * cos螖位) * t), sin蠁0 * sin蠁 + cos蠁0 *
        cos蠁 * cos螖位);
  ...
  function d3_geo_conicConformal(蠁0, 蠁1) {
    var cos蠁0 = Math.cos(蠁0),
      ...
      n = 蠁0 === 蠁1 ? Math.sin(蠁0) : Math.log(cos蠁0 / Math.cos(蠁1)) / Math.log(
        t(蠁1) / t(蠁0)),
    ...
    forward.invert = function(x, y) {
      ...
      return [Math.atan2(x, 蟻0_y) / n, 2 * Math.atan(Math.pow(F / 蟻, 1 / n)) -
        half蟺
      ];
    };
    return forward;
  ...
  function d3_geo_conicEquidistant(蠁0, 蠁1) {
    ...
    forward.invert = function(x, y) {
      var 蟻0_y = G - y;
      return [Math.atan2(x, 蟻0_y) / n, G - d3_sgn(n) * Math.sqrt(x * x + 蟻0_y *
        蟻0_y)];
    };
    return forward;
  }
  ...
  function d3_geom_voronoiRemoveBeach(beach) {
    ...
    var lArc = previous;
    while (lArc.circle && abs(x - lArc.circle.x) < 蔚 && abs(y - lArc.circle.cy) <
      蔚) {
      previous = lArc.P;
    ...
    var rArc = next;
    while (rArc.circle && abs(x - rArc.circle.x) < 蔚 && abs(y - rArc.circle.cy) <
      蔚) {
      next = rArc.N;
  ...
  d3.interpolators = [function(a, b) {
    ...
    return (t === "string" ? d3_rgb_names.has(b) || /^(#|rgb\(|hsl\()/.test(
        ...
        "object" && isNaN(b) ? d3_interpolateObject : d3_interpolateNumber)
      (a, b);
  }];
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      var r0 = innerRadius.apply(this, arguments),
        ...
        c1 = Math.cos(a1),
        s1 = Math.sin(a1);
      return da >= d3_svg_arcMax ? r0 ? "M0," + r1 + "A" + r1 + "," + r1 +
        " 0 1,1 0," + -r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + r1 + "M0," +
        r0 + "A" + r0 + "," + r0 + " 0 1,0 0," + -r0 + "A" + r0 + "," + r0 +
        " 0 1,0 0," + r0 + "Z" : "M0," + r1 + "A" + r1 + "," + r1 +
        " 0 1,1 0," + -r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + r1 + "Z" : r0 ?
        "M" + r1 * c0 + "," + r1 * s0 + "A" + r1 + "," + r1 + " 0 " + df +
        ",1 " + r1 * c1 + "," + r1 * s1 + "L" + r0 * c1 + "," + r0 * s1 + "A" +
        r0 + "," + r0 + " 0 " + df + ",0 " + r0 * c0 + "," + r0 * s0 + "Z" :
        "M" + r1 * c0 + "," + r1 * s0 + "A" + r1 + "," + r1 + " 0 " + df +
        ",1 " + r1 * c1 + "," + r1 * s1 + "L0,0" + "Z";
    }
  ...
  function d3_svg_lineCardinalOpen(points, tension) {
    return points.length < 4 ? d3_svg_lineLinear(points) : points[1] +
      d3_svg_lineHermite(points.slice(1, points.length - 1),
  ...
  d3.transition = function(selection) {
    return arguments.length ? d3_transitionInheritId ? selection.transition() :
      selection : d3_selectionRoot.transition();
  };
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n;) {
        if (node = group[i]) {
          ...
          for (var k = -1, o = subnodes.length; ++k < o;) {
            if (subnode = subnodes[k]) d3_transitionNode(subnode, k, id,
              transition);
            subgroup.push(subnode);
          }
        }
  ...
  d3_transitionPrototype.tween = function(name, tween) {
    var id = this.id;
    if (arguments.length < 2) return this.node().__transition__[id].tween.get(
      name);
  ...
  function d3_transition_tween(groups, name, value, tween) {
    ...
    return d3_selection_each(groups, typeof value === "function" ? function(
      node, i, j) {
      node.__transition__[id].tween.set(name, tween(value.call(node, node.__data__,
        i, j)));
  ...
  d3_transitionPrototype.styleTween = function(name, tween, priority) {
    ...
    function styleTween(d, i) {
      var f = tween.call(this, d, i, d3_window.getComputedStyle(this, null).getPropertyValue(
        name));
      return f && function(t) {
  ...
  d3_transitionPrototype.remove = function() {
    return this.each("end.transition", function() {
      var p;
      if (this.__transition__.count < 2 && (p = this.parentNode)) p.removeChild(
        this);
    });
  };
  ...
  d3_transitionPrototype.delay = function(value) {
    ...
    return d3_selection_each(this, typeof value === "function" ? function(
      node, i, j) {
      node.__transition__[id].delay = +value.call(node, node.__data__, i,
        j);
  ...
  d3_transitionPrototype.duration = function(value) {
    ...
    return d3_selection_each(this, typeof value === "function" ? function(
      node, i, j) {
      node.__transition__[id].duration = Math.max(1, value.call(node,
        node.__data__, i, j));
          ...
          case "bottom":
            {
              ...
              textEnter.attr("y", Math.max(innerTickSize, 0) +
                tickPadding);
              lineUpdate.attr("x2", 0).attr("y2", innerTickSize);
              textUpdate.attr("x", 0).attr("y", Math.max(innerTickSize, 0) +
                tickPadding);
              text.attr("dy", ".71em").style("text-anchor", "middle");
              pathUpdate.attr("d", "M" + range[0] + "," + outerTickSize +
                "V0H" + range[1] + "V" + outerTickSize);
              break;
            }
          ...
          case "left":
            {
              ...
              textEnter.attr("x", -(Math.max(innerTickSize, 0) +
                tickPadding));
              lineUpdate.attr("x2", -innerTickSize).attr("y2", 0);
              textUpdate.attr("x", -(Math.max(innerTickSize, 0) +
                tickPadding)).attr("y", 0);
              text.attr("dy", ".32em").style("text-anchor", "end");
              pathUpdate.attr("d", "M" + -outerTickSize + "," + range[0] +
                "H0V" + range[1] + "H" + -outerTickSize);
              break;
            }
